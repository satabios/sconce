...
Generic structural pruner for Transformer models (ViT, LLM, any architecture).

Supports:
- Fused QKV projections  (timm ViT, GPT-2, Falcon …)
- Separate Q/K/V projections  (EVA02, LLaMA, Mistral, Qwen2, BERT …)
- Grouped Query Attention (GQA)  — K/V sized by num_kv_heads, Q by num_heads
- Standard FFN  (fc1/fc2)
- SwiGLU FFN  (gate_proj+up_proj+down_proj, or fc1_g+fc1_x+fc2)
- GPU-parallel block-level sensitivity scan
- No torch_pruning dependency — pure PyTorch
"""
...
# GPU-discovery helpers live in pruner.py as module-level functions
from .pruner import _nvidia_smi_free_gb, _measure_model_vram_gb, _build_gpu_worker_pool
